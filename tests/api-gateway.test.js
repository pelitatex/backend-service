...
const forwardToMicroservice = {
    microservice1: 'http://localhost:3301',
    // Add more microservices as needed
};

describe('Test /graphql endpoint', () => {
    
    test('It should require tenant and microservice header', async () => {
        const response = await request(apiGateway)
            .get('/graphql')
            .set('x-tenant', 'testapi001');
        expect(response.statusCode).toBe(400);
        expect(response.body.error).toBe('Missing tenant or Microservice in headers');
    });
